...
most_active_requestors () {
  #local LIST_OF_CLIENTS=`egrep -i '\sconnection from\s' $ACCESSLOG | awk -F\: '{ print $NF }' | awk '{ print $(NF-2) }' | grep -v unknown | sort | uniq -w11 `
  ...
  while IFS= read -r line; do
    ...
    #local COUNT=`egrep -i "\sconn\=($LIST_OF_CONNECITONS)\s" $ACCESSLOGPATH/$ACCESSLOG | egrep -c '\sSRCH\s'`
